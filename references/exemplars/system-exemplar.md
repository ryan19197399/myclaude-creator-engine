...
**Demonstrates:** SYSTEM.md with routing layer, `.claude-plugin/plugin.json`, shared knowledge base,
...
See `.claude-plugin/plugin.json` and `config/routing.yaml`.
...
When a component is disabled in `plugin.json`, the system skips that step:
...
```json
{
  ...
  "version": "1.1.0",
  "description": "Transforms research requests into structured intelligence reports with automated source discovery, synthesis, fact-checking, and report assembly.",
  "author": "@researchcraft",
  ...
  "install": {
    "target": ".claude/skills/research-intelligence/",
    "post_install": []
  },
  "compatibility": {
    "claude_code": ">=1.0.0"
...
- [x] `.claude-plugin/plugin.json` with full component specification
