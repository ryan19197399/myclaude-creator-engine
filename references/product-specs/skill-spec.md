...
The frontmatter block controls how Claude Code discovers and activates the skill.

**Required fields:**
| Field | Purpose | Example |
|-------|---------|---------|
| `name` | Skill slug (matches filename) | `my-skill` |
| `description` | What it does + when to use (shown in / autocomplete) | `"Analyzes code for security. Use when: reviewing PRs."` |

**Optional frontier fields:**
| Field | Purpose | Default | Example |
|-------|---------|---------|---------|
| `allowed-tools` | Restrict available tools | all tools | `Read, Grep, Glob` |
| `model` | Force a specific model | inherited | `claude-sonnet-4-6` |
| `effort` | Force reasoning effort | inherited | `high` |
| `context` | Subagent isolation mode | shared | `fork` |
| `paths` | Conditional activation (glob patterns) | always active | `['src/**/*.ts']` |
| `user-invocable` | Show in / autocomplete menu | `true` | `false` |
| `disable-model-invocation` | Prevent Claude from auto-triggering | `false` | `true` |
| `argument-hint` | Autocomplete hint text | none | `"[--fix] [slug]"` |
| `hooks` | Skill-scoped hooks (only fire when active) | none | see hooks-spec.md (not yet empirically verified — test before using) |
...
2. Read `examples/examples.md` — [what exemplars teach]
...
**Skill-Specific:**
- [ ] `examples/examples.md` with 3+ concrete examples
